...
| `└─ generation_mode` |    ✗     | Controls overall motion style. _ `realistic` - Maintains likeness well, high quality, and reliable. _ `prompted` - Slightly lower likeness; allows option to prompt scene. **Deprecated values (maintained for backward compatibility):** _ `pro` - Deprecated: use `realistic` _ `standard` - Deprecated: use `prompted` _ `stable` - Deprecated: use `realistic` _ `expressive` - Deprecated: use `prompted` | `"realistic"`                                                                                |
