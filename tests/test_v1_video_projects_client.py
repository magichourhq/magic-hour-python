...
def test_get_200_success_all_params():
    ...
    response = client.v1.video_projects.get(id="cuid-example")
    try:
...
async def test_await_get_200_success_all_params():
    ...
    response = await client.v1.video_projects.get(id="cuid-example")
    try:
        pydantic.TypeAdapter(models.V1VideoProjectsGetResponse).validate_python(
            response
        )
...
def test_delete_204_success_all_params():
    ...
    # tests calling sync method with example data
    client = Client(token="API_TOKEN", environment=Environment.MOCK_SERVER)
    response = client.v1.video_projects.delete(id="cuid-example")
...
async def test_await_delete_204_success_all_params():
    ...
    # tests calling async method with example data
    client = AsyncClient(token="API_TOKEN", environment=Environment.MOCK_SERVER)
    response = await client.v1.video_projects.delete(id="cuid-example")
    assert response is None
